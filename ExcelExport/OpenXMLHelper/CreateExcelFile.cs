...
namespace ExcelTest
{
    ...
    public class CreateExcelFile
    {
        ...
        //    return CreateExcelDocument(ds, xlsxFilePath);
        //}
        public void Export(DataTable dt, string xlsxFilePath)
        {
            ...
            {
                SpreadsheetDocument document = SpreadsheetDocument.Open(xlsxFilePath, true);

                CreateExistExcelDocument(dt, xlsxFilePath, document);
            }
            else
            {
        ...
        public static void CreateExistExcelDocument(DataTable dt, string excelFilename, SpreadsheetDocument document)
        {
            WorksheetPart newWorksheetPart = null;
            try
            {
                Sheet sheet = document.WorkbookPart.Workbook.Descendants<Sheet>().FirstOrDefault(s => s.Name == dt.TableName);
                if (sheet != null)
                {
                    newWorksheetPart = (WorksheetPart)document.WorkbookPart.GetPartById(sheet.Id);
                    sheet.Remove();
